...
test.describe('New Stars Radio - Mobile Experience', () => {
  ...
  test('should handle volume control on mobile', async ({ page }) => {
    ...
    // Test mute button
    const muteButton = page.locator('[aria-label*="mute"]').first();
    await muteButton.tap();
    
    // Should show 0 volume when muted
  ...
  test('should display metadata and update automatically', async ({ page }) => {
    ...
    // Test metadata refresh (wait for automatic update)
    await page.waitForTimeout(11000); // Wait for next metadata update
    
    // Ensure metadata is still present after update
    await expect(currentSongElement).not.toBeEmpty();
  });
  ...
  test('should handle poor network conditions', async ({ page, context }) => {
    ...
    await context.route('**/*', async (route, request) => {
      if (request.url().includes('api') || request.url().includes('embed')) {
        // Delay API calls by 2 seconds to simulate poor connection
    ...
    // App should still load, even with slow network
    await expect(page.locator('h1')).toContainText('New Stars Radio', { timeout: 15000 });
    
    // Loading state should be visible during slow metadata fetch
    await expect(page.locator('[data-testid="loading"]')).toBeVisible();
    
    // Eventually metadata should load
    await expect(page.locator('[data-testid="current-song"]')).toBeVisible({ timeout: 20000 });
  });
  ...
  test('should work offline after initial load', async ({ page, context }) => {
    ...
    // App should still be functional (cached content)
    await expect(page.locator('h1')).toContainText('New Stars Radio');
    
    // Play button should still work (though streaming won't)
    const playButton = page.locator('[aria-label="Play"]').first();
    await playButton.tap();
    // Button should respond even if stream doesn't work
